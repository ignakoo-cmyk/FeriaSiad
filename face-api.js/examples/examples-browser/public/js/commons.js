// Hack para corregir el texto invertido creado por el CSS mirror
...
CanvasRenderingContext2D.prototype.fillText = function(text, x, y, maxWidth) {
  ...
  if (maxWidth !== undefined) {
      originalFillText.call(this, text, 0, 0, Math.abs(maxWidth));
  } else {
      originalFillText.call(this, text, 0, 0);
  }
  this.restore();
};
...
function renderNavBar(navbarId, exampleUri) {
  const examples = [
    ...
    {
      uri: 'webcam_career_prediction',
      name: 'Predicción de Carreras UAH'
    }
  ]
  ...
  menuButton.href='#'
  menuButton.id = 'menu-toggle-btn'
  menuButton.classList.add('button-collapse', 'show-on-large', 'btn', 'waves-effect', 'waves-light')
  menuButton.setAttribute('data-activates', 'slide-out')
  menuButton.innerHTML = '<i class="material-icons left">menu</i> Menú'
  navbar.appendChild(menuButton)

  const li = document.createElement('li')
  const githubLink = document.createElement('div')
  githubLink.classList.add('side-by-side')
  githubLink.id = 'github-link'
  githubLink.style.paddingTop = '20px'
  const logoImg = document.createElement('img')
  logoImg.src = 'logo_uah.svg'
  logoImg.style.width = '260px'
  githubLink.appendChild(logoImg)
  // Disable the github link behavior conceptually or remove icon if needed
